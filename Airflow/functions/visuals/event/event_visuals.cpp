...
void c_event_visuals::on_bullet_impact(c_game_event* event) {
	...
	if (player == g_ctx.local) {
		if (g_cfg.misc.impacts) {
			auto clr = g_cfg.misc.server_clr;
	...
	for (int i = 0; i < hitmarkers.size(); ++i) {
		...
		if (hit.alpha > 0.f) {
			...
			if (g_cfg.misc.hitmarker & 1) {
				auto clr = g_cfg.misc.hitmarker_clr;
			...
			if (g_cfg.misc.damage) {
				auto clr = g_cfg.misc.damage_clr;
...
void c_event_visuals::on_render_start(int stage) {
	...
	if (g_cfg.misc.impacts) {
		auto clr = g_cfg.misc.client_clr;
		...
		for (auto i = 0; i < 65; i++) {
			...
			if (a.size() > 0) {
				...
				if (player) {
					...
					color trace_clr = color{ 255, 255, 255 };
					if (is_local)
