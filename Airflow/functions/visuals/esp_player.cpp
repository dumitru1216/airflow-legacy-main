...
void render_main_esp(c_esp_store::player_info_t* info) {
	...
	if (enemy_esp.elements & 2)
		info->objs[iter++] = esp_objects_t{ true, esp_up,  enemy_esp.colors.name, 0.f, 0.f, info->name, font_default, true, info->dormant_alpha };
	...
	if (enemy_esp.elements & 8)
		info->objs[iter++] = esp_objects_t{ true, esp_down,  enemy_esp.colors.weapon, 0.f, 0.f, info->weaponicon, font_icon, true, info->dormant_alpha };

	if (enemy_esp.elements & 16)
		info->objs[iter++] = esp_objects_t{ true, esp_down,  enemy_esp.colors.weapon, 0.f, 0.f, info->weaponname, font_pixel, false, info->dormant_alpha };

	if (enemy_esp.elements & 32 && !info->miscweapon)
		info->objs[iter++] = esp_objects_t{ true, esp_down,  enemy_esp.colors.ammo_bar, info->ammo, info->maxammo, "", 0, false, info->dormant_alpha };
	...
	if (info->dormant_alpha >= 0.9f && (enemy_esp.elements & 512)) {
		auto clr = enemy_esp.colors.skeleton;
...
void render_offscreen_esp(c_esp_store::player_info_t* info) {
	...
	auto clr = enemy_esp.colors.offscreen_arrow;
